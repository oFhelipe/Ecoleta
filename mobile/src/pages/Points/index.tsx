  ...
  const Points = () =>{
    ...
    useEffect(()=>{
      ...
      async function loadPosition(){
        ...
        const { status } = await await Location.requestPermissionsAsync();
          ...
          <ScrollView 
              ...
              {items.map((item)=>{
                return(
                  ...
                  <TouchableOpacity activeOpacity={0.6} key={item.id} style={[styles.item, selectedItems.includes(item.id) ? styles.selectedItem : {}]} onPress={()=>{handleSelectItem(item.id)}}>
